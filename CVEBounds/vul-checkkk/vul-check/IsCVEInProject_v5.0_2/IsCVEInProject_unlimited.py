...
def prompt():
    print('''参数错误
Python iscvinproject.py 源代码绝对路径 被测系统根目录 测试的版本编号前两位 new
Python iscvinproject.py 源代码绝对路径 被测系统根目录 测试的版本编号前两位 cont cve编号''')
    sys.exit(0)
...
if __name__ == '__main__':
    ...
    try:
        ...
        if not re.match(r'\d+\.\d+', version_num):
            prompt()
        # 启动命令，new或者cont
        order = sys.argv[4]
    ...
    kernelPub_time = generateResult.get_targetMINPubTime(version_num, os.path.join(relative_path,'Linux Kernel Release Time.xls'))
    ...
    try:
        print(kernelPub_time)
        pub_year = int(kernelPub_time.split('-')[0])
    except Exception:
        prompt()
    # print(path_list)
    if str(order) == 'new':
