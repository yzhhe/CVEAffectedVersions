...
def get_Condition(cve_name,relative_path):
    version_path = os.path.join(relative_path,'all_cve_version2')
    if not os.path.exists(os.path.join(version_path,cve_name+'.txt')):
        cve_version = ''
    else:
        cve_version = Repository.get_file_line_list(os.path.join(version_path, cve_name + '.txt'))[0].split(' ')[-1].strip()
    time_path = os.path.join(version_path, 'publish_date')
    if not os.path.exists(os.path.join(time_path,cve_name+'.txt')):
        pub_time = ''
    else:
        pub_time = Repository.get_file_line_list(os.path.join(time_path, cve_name + '.txt'))[0].strip()
    return [cve_version,pub_time]
...
def prompt():
    print('''参数错误
Python isCVEInProject_limited.py 源代码绝对路径 被测系统根目录 测试的版本编号前两位 new
Python isCVEInProject_unlimited.py 源代码绝对路径 被测系统根目录 测试的版本编号前两位 cont cve编号''')
    sys.exit(0)
...
if __name__ == '__main__':
    ...
    try:
        #源代码的绝对路径
        sourceCode_path = sys.argv[1]
        if '/' not in sourceCode_path:
            prompt()
        #相对路径
        ...
        if '/' not in relative_path:
            prompt()
        # 版本编号
        kernel_version_num = sys.argv[3]
        if not re.match(r'\d+\.\d+', kernel_version_num):
            prompt()
        # 启动命令，new或者cont
        order = sys.argv[4]
        if order!= 'new' and order!='cont':
            prompt()
        #如果是cont就有
        if order == 'cont':
            cve_num = sys.argv[5]
    ...
    cve_name = os.path.join(relative_path,'Linux Kernel Patch_v2.0')
    if not os.path.exists(cve_name):
        print('您输入的路径不存在，请重新输入')
        sys.exit(0)
    path_list=os.listdir(cve_name)
    path_list.sort()
    #要测试的linux版本
    # kernel_edition_num = 'linux-' + version_num
    #版本号精确到2位
    kernelPub_time = generateResult.get_targetMINPubTime(kernel_version_num, os.path.join(relative_path,'Linux Kernel Release Time.xls'))
    #版本号精确到3位
    # kernelPub_time = generateResult.get_targetPubTime(kernel_version_num, os.path.join(relative_path,'Linux Kernel Release Time.xls'))
    print(kernelPub_time)
    if kernelPub_time == None :
        print('此版本号的发布时间不存在')
        sys.exit(0)
    print(os.path.join(relative_path,'Linux Kernel Release Time.xls'))
    try:
        print(kernelPub_time)
        kernel_pub_year = int(kernelPub_time.split('-')[0])
    except Exception:
        prompt()
    # print(path_list)
    ...
    if str(order) == 'new':
        ...
        for index,file_name in enumerate(path_list):
            if os.path.isdir(os.path.join(cve_name, file_name)):
                ...
                versionAndPubtime = get_Condition(file_name, relative_path)
                restrictVersionNum = versionAndPubtime[0]
                cvePubTime = versionAndPubtime[1]
                print("%s的版本及发布时间为%s"%(file_name,versionAndPubtime))
                print("kernel的版本及发布时间为%s和%s" % (kernel_version_num, kernel_pub_year))
                if restrictVersionNum !='' and cvePubTime !='':
                    if kernel_version_num <= restrictVersionNum and kernelPub_time <= cvePubTime:
                        print('path1')
                        is_cve_in_project(cve_dir_path, sourceCode_path)
                elif restrictVersionNum == '' and cvePubTime !='':
                    if  kernelPub_time <= cvePubTime:
                        print('path2')
                        is_cve_in_project(cve_dir_path, sourceCode_path)
                elif restrictVersionNum != '' and cvePubTime =='':
                    if  kernel_version_num <= restrictVersionNum and year >= kernel_pub_year:
                        print('path3')
                        is_cve_in_project(cve_dir_path, sourceCode_path)
                elif restrictVersionNum == '' and cvePubTime =='':
                    if year >= kernel_pub_year :
