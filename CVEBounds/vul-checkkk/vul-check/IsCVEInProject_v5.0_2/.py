...
def prompt():
    print('''参数错误
Python iscvinproject.py 被测系统根目录 测试的版本编号 new
Python iscvinproject.py 被测系统根目录 测试的版本编号 cont cve编号''')
    sys.exit(0)
...
if __name__ == '__main__':
    ...
    kernelPub_time = generateResult.get_targetPubTime(kernel_edition_num, os.path.join(relative_path,'Linux Kernel Release Time.xls'))
    try:
        ...
        for index,file_name in enumerate(path_list):
            if os.path.isdir(os.path.join(cve_name, file_name)):
                ...
                if year >= pub_year :
                        ...
                        is_cve_in_project(cve_dir_path,os.path.join(relative_path,kernel_edition_num))
    elif str(order) == 'cont':
